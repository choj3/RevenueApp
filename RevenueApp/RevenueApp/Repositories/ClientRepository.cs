...
public class ClientRepository: IClientRepository
{
    ...
    public async Task<PrivateClient?> GetPrivateClientByIdAsync(int privateClientId, CancellationToken token)
    {
        return await _distributionDbContext.PrivateClients.Include(p => p.Clients)
            .FirstOrDefaultAsync(p => p.PrivateClientId == privateClientId, token);
    }
}